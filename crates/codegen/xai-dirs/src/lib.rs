//! Home-directory resolution generally: USERPROFILE-first `home_dir`, plus the
//! gcode home: `$GCODE_HOME`, the legacy `$GROK_HOME`, or `<home>/.gcode`.
...
pub enum GrokHomeSource {
    /// A non-empty `$GCODE_HOME` or legacy `$GROK_HOME` override.
    EnvOverride,
    /// `<home>/.gcode` derived from the home directory.
    HomeDefault,
}
...
/// `$GCODE_HOME` verbatim when non-empty, then legacy `$GROK_HOME`, else
/// `<home>/.gcode`. The env value is
...
mod tests {
    ...
    fn empty_env_falls_through_to_os_home() {
        ...
        assert_eq!(
            resolved,
            Some((
                dunce::canonicalize(tmp.path()).unwrap().join(".gcode"),
    ...
    fn none_when_nothing_resolves() {
        assert_eq!(
            resolve_grok_home_from(
                /* gcode_home_env */ None, /* grok_home_env */ None,
                /* os_home */ None,
            ),
            None
        );
    ...
    #[test]
    fn gcode_home_wins_over_legacy_grok_home() {
        assert_eq!(
            resolve_grok_home_from(
            ...
            Some((PathBuf::from("/gcode/home"), GrokHomeSource::EnvOverride))
        );
    }
}
