...
#[test]
fn test_reinstall_hint_internal_all_channels_stay_on_gcode_fork() {
    for channel in ["stable", "alpha", "enterprise"] {
        let hint = reinstall_hint("internal", channel);
        assert!(hint.contains("yuzhi535/gcode"), "channel {channel}: {hint}");
        assert!(!hint.contains("x.ai/cli"), "channel {channel}: {hint}");
        assert!(!hint.contains("GROK_CHANNEL"), "channel {channel}: {hint}");
    }
}
